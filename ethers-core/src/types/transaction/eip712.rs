...
pub fn encode_eip712_type(token: Token) -> Token {
    match token {
        ...
        Token::FixedBytes(t) => {
            ...
            Token::Uint(U256::from_big_endian(&padded))
        }
        ...
        Token::Array(tokens) => Token::Uint(U256::from_big_endian(&keccak256(abi::encode(
            &tokens.into_iter().map(encode_eip712_type).collect::<Vec<Token>>(),
        )))),
        Token::FixedArray(tokens) => Token::Uint(U256::from_big_endian(&keccak256(abi::encode(
            &tokens.into_iter().map(encode_eip712_type).collect::<Vec<Token>>(),
        )))),
        Token::Tuple(tuple) => {
